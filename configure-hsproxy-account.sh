...
CHECK_IRODS_ZONE=${CHECK_IRODS_ZONE%?}

if [ ${CHECK_IRODS_ZONE} -ne ${HYDORSHARE_IRODS_ZONE} ]; then
...
#iadmin mkresc hydrodevResc 'unixfilesystem' localhost:/var/tmp/Vault
#iadmin mkuser hsproxy rodsuser
#iadmin moduser hsproxy password proxywater1

# check resource name
CHECK_IRODS_RESOURCE=$(docker exec -ti hs-irods-icat sudo -u irods iadmin lr | grep ${HYDROSHARE_IRODS_RESOURCE})
CHECK_IRODS_RESOURCE=${CHECK_IRODS_RESOURCE%?}
...
if [ -z ${CHECK_IRODS_USER} ]; then
    ...
    docker exec -ti hs-irods-icat sudo -u ${SERVICE_ACCT_USERNAME} iadmin moduser ${HYDROSHARE_IRODS_USERNAME} \
        password ${HYDROSHARE_IRODS_AUTH}
else
